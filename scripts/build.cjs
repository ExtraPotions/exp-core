...
const manifest = { coreVersion:'3.2.21', source:provenance, bundleSha256:hash(source), files:Object.fromEntries(files.map(f=>[f,hash(normalize(fs.readFileSync(path.join(root,'src',f),'utf8')))])) };
...
console.log('Core 3.2.21 '+manifest.bundleSha256);
