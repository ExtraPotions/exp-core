...
for(const viewport of [{width:1280,height:900},{width:360,height:640}])test(`${viewport.width}px: Dropper progress opens inward at either anchor without moving peers out of the three-column grid`,async t=>{
 ...
 const assertHitTargets=async()=>{const hits=await page.evaluate(()=>[...document.querySelectorAll('[data-exp-product-launcher="1"]')].map(host=>{const launcher=host.shadowRoot.querySelector('.launcher,.ward-launcher,#tdh-settings-launcher'),box=launcher.getBoundingClientRect(),hit=document.elementFromPoint(box.left+box.width/2,box.top+box.height/2);return {id:host.dataset.productId,hit:hit?.dataset?.productId||hit?.id||hit?.tagName||null};}));for(const hit of hits)assert.equal(hit.hit,hit.id,JSON.stringify(hits));};
